...
class CustomLoss(torch.nn.Module):
    def __init__(self, params):
        ...
        self.row_coef:float = params['row_coef']

    def forward(self, pred_dict, true_dict):
        n_pred = pred_dict["node_classes"]
        e_pred = pred_dict["E_pred"]
        row_pred = pred_dict["row_pred"]
        n_true = true_dict["node_classes"]
        e_true = true_dict["E_true"]

        is_rows = (e_true == 1).to(torch.float32)
        is_block = (e_true >0.5).to(torch.float32)
       
        loss =  self.edge_coef*self.bce(e_pred, is_block) + \
                self.node_coef*self.ce(n_pred, n_true) + \
                self.row_coef*self.bce(row_pred, is_rows)
        return loss
