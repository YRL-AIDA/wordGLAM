    ...
    def is_one_block(word1, word2, blocks):
        for block in blocks:
            if block.is_intersection(word1) and block.is_intersection(word2):
                y1t, y1b =  word1.y_top_left, word1.y_bottom_right
                y2t, y2b =  word2.y_top_left, word2.y_bottom_right
                if y1b < y2t or y2b < y1t:  
                    return 0.75
                return 1
        return 0
...
def extract(path_dataset, path_img_publaynet=None, path_pdf_publaynet=None):
    ...
    for i, file in enumerate(files):
        ...
        try:
            ...
            # json_with_featchs.add_featchs(lambda: nodes_feature_new_styles(json_with_featchs.json['nodes_feature']), names=['nodes_feature', 'old_nodes_feature'],
            #                     is_reupdate=True, rewrite=True) 
