...
from pager.page_model.sub_models.phisical_model.converter_graph_model.words_and_styles2sp_graph import Word2Vec
import numpy as np
import os
...
def featch_A_and_EdgesFeatures(words, w_img, h_img):
    def get_graph_from_segments(segments):    
        def get_edges_rows(left_keys, right):
            edges_rows = []
            for i, index in enumerate(left_keys):
                l, k = index
                l0, l1 = max(0, l+1), min(N-1, l-1)
                ki = k
                while ki > 0:
                    if(len(right[l][ki]) != 0):
                        a = right[l][ki][0]    
                    elif(ki-1>=0 and len(right[l][ki-1]) != 0):
                        a = right[l][ki-1][0]
                    elif(len(right[l0][ki]) != 0):
                        a = right[l0][ki][0]
                    elif(len(right[l1][ki]) != 0):
                        a = right[l1][ki][0]
                    else:
                        ki-=1
                        continue
                    edges_rows.append((i, a))
                    break
            return edges_rows

        def get_edges_cols(indexs_keys, indexs):
            edges_cols = []
            for i, index in enumerate(indexs_keys):
                l, k = index
                k0, k1 = max(0, k-1),min(M-1, k+1)
                li = l
                while li < N:
                    if(len(indexs[li][k]) > 1):
                        a = indexs[li][k][1]
                    elif((li+1 < N) and len(indexs[li+1][k]) != 0):
                        a = indexs[li+1][k][0]
                    elif(len(indexs[li][k0]) != 0):
                        a = indexs[li][k0][0]
                    elif(len(indexs[li][k1]) != 0):
                        a = indexs[li][k1][0]
                    else:
                        li+=1
                        continue
                    edges_cols.append((i, a))
                    break
            return edges_cols
        ...
        for seg in segments:
            ...
            xl, xr = seg.x_top_left, seg.x_bottom_right
            points.append([(xl, yc), (xc, yc), (xr, yc)])
        h_mean = np.mean(H)
        N = int(h_img//h_mean)+1
        M = int(w_img//h_mean)+1


        indexs = [[[[] for j in range(M)] for i in range(N)] for _ in range(3)]
        indexs_keys = [[] for _ in range(3)]
        ...
        for i, p in enumerate(points):
            for j, pi in enumerate(p):
                ...
                k = int(x_pi//h_mean)
                l = int(y_pi//h_mean)
                indexs[j][l][k].append(i)
                indexs_keys[j].append((l, k))

        left, center, right = indexs[0],indexs[1],indexs[2]
        left_keys, center_keys, right_keys =indexs_keys[0],indexs_keys[1],indexs_keys[2]
        edges_rows = get_edges_rows(left_keys, right)
        
        edges_left = get_edges_cols(left_keys, left)
        edges_center = get_edges_cols(center_keys, center)
        edges_right = get_edges_cols(right_keys, right)
        return edges_rows, edges_left, edges_center, edges_right
    
    segments = [Word(w).segment for w in words]
    edgess = get_graph_from_segments(segments)
    A = []
    edges_featch = []
    i = 0
    index_dict = {}
    for j, edges in enumerate(edgess):
        for  e in edges:
            if e in A:
                ii = index_dict[e]
                edges_featch[ii][j] = 1.0
            else:
                index_dict[e] = i
                vec = [0 for _ in range(6)]
                w1, w2 = segments[e[0]],segments[e[1]] 
                vec[j] = 1.0
                vec[4] = w1.get_angle_center(w2)
                vec[5] = w1.get_min_dist(w2)
                edges_featch.append(vec)
                A.append(e)
                i+=1
    return [
        [[a[0] for a in A],
...
class JsonWithFeatchsExtractor(BaseExtractor):
    def extract(self, json_with_featchs: JsonWithFeatchs):
        print(json_with_featchs.name_file)
        ...
        if not 'pages' in img2words_and_styles.page_units[0].sub_model.pdf_json:
            json_with_featchs.add_featchs(lambda: [[[],[]], [], []], names=['A', 'edges_feature', 'nodes_feature'],
                            is_reupdate=False, rewrite=False)
            return
                                          
        page = img2words_and_styles.page_units[0].sub_model.pdf_json["pages"][0]
        json_with_featchs.add_featchs(lambda: featch_A_and_EdgesFeatures(json_with_featchs.json['words'],
                                                                         page['width'], 
                                                                         page['height']), names=['A', 'edges_feature'], 
                            is_reupdate=False, rewrite=False)
