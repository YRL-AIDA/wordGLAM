...
def get_tensor_from_graph(graph):
    edge_index = torch.tensor(graph["A"], dtype=torch.long)
    ...
    node_x = torch.tensor(data=nodes_feature, dtype=torch.float32)
    edge_raw_tensor = torch.tensor(data=edge_raw, dtype=torch.float32)
    sp_A = torch.sparse_coo_tensor(indices=edge_index, values=v_in, size=(N, N), dtype=torch.float32)
    return node_x, edge_raw_tensor, sp_A, edge_index
...
class Json2Blocks(WordsAndStylesToGLAMBlocks):
    ...
    def segmenter(self, graph) -> List[int]:
        ...
        node_x, edge_raw, sp_A, edge_index = tensors
        N = len(node_x)
        ...
        if N == 1:
            ...
            return np.array([0 for _ in edge_index[0]])
        outputs = self.model(node_x, edge_raw, edge_index)
        edge_multi_class = outputs["edge_multi_class"]
        edge_bin_class = outputs["edge_bin_class"]
        
        rez = np.zeros_like(edge_bin_class.detach().numpy())
        self.tmp = edge_multi_class.detach().numpy()
        self.tmp_i = edge_index.detach().numpy().transpose().tolist()
        rez[edge_bin_class>0.5] = 1
        return rez
    ...
    def set_label_output_block(self, output_model, words, graph):
        # CLASSIFIER +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        edge_words = [[] for _ in words]
        for i, e in enumerate(self.tmp_i):
            edge_words[e[0]].append(i)
            edge_words[e[1]].append(i)

        for block in output_model.blocks:
            block_edges = []
            for i, word in enumerate(words):
                if block.segment.is_intersection(word.segment):
                    for e in edge_words[i]:
                        block_edges.append(e)
            block_edges = [self.tmp[i] for i in set(block_edges)]
            # print(block_edges)
            class_ = int(np.argmax(np.array(block_edges).mean(axis=0)))
