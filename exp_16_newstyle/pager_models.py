...
def nodes_feature(styles, words):
    ...
    coord_vec = np.array([get_vec_coord(w) for w in words], dtype=np.float32)
    h_max = max(coord_vec[:,4])
    w_max = max(coord_vec[:,1])
    coord_vec[:, :3] = coord_vec[:, :3] / w_max
    coord_vec[:, 3:] = coord_vec[:, 3:] / h_max
...
def nodes_feature_new_styles(styles, words, nodes_feature):
    ...
    coord_vec = np.array([get_vec_coord(w) for w in words], dtype=np.float32)#rez[:, :6]
    h_max = max(coord_vec[:,4])
    w_max = max(coord_vec[:,1])
    coord_vec[:, :3] = coord_vec[:, :3] / w_max
    coord_vec[:, 3:] = coord_vec[:, 3:] / h_max
...
def edges_feature(A, words):
    ...
    for i, j in zip(A[0], A[1]):
        ...
        edges_featch.append([w1.get_angle_center(w2), 1/w1.get_min_dist(w2)])
    # print(edges_featch)
    return [edges_featch]
