...
def step(model: torch.nn.Module, batch, optimizer, criterion, train=True):
    ...
    for j, graph in enumerate(batch):
        try:
            X, Y, sp_A, E_true, N_true, C_true, i = get_tensor_from_graph(graph)
            if len(X) in (0, 1):                       
                continue

            Node_class, E_pred, C_pred = model(X, Y, sp_A, i)
            loss = criterion(Node_class.to(device), N_true, E_pred.to(device), E_true, C_pred.to(device), C_true)
